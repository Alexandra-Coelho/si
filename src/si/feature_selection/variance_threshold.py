...
class VarianceThreshold:
    ...
    def transform(self, dataset: Dataset) -> Dataset:
        ...
        """
        X = dataset.X
        features_mask = self.variance > self.threshold #seleciona colunas que têm var superior ao threshold
        X = X[:, features_mask] #obrigatorio retornar sempre um novo dataset
        features = np.array(dataset.features)[features_mask] #isto porque é uma lista de strings
        return Dataset(X=X, y=dataset.y, features=list(features), label=dataset.label) #ja nao tem colunas com pouca var
...
if __name__ == "__main__":
    ...
    dataset = selector.transform(dataset) #aplica uma mask consoante o threshold
